...
export async function deploy() {
  ...
  const projectName = config.app?.name?.toLowerCase().replace(/\s+/g, "-") ?? "manic-app";

  const deployCommands: Record<string, { configFile: string | null; command: string }> = {
    vercel: {
  ...
  for (const provider of providers) {
    ...
    if (!deployInfo) {
      console.log(
        eventLine("deploy", `unknown provider "${provider.name}", run deployment manually`, "warn"),
      );
      continue;
    }
