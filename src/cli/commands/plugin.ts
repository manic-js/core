...
function parsePluginList(config: string): string[] {
  ...
  for (const line of importLines) {
    const m = line.match(/^import\s+(.+)\s+from\s+["']([^"']+)["'];?$/u);
    if (!m) continue;
...
export async function plugin(args: string[]): Promise<void> {
  ...
  console.log(
    `  ${dim('Actions:')} ${cyan('list')} | ${cyan('add <package>')} | ${cyan('remove <package>')}`
  );
  console.log(divider());

  if (!action || !['add', 'remove', 'list'].includes(action)) {
    console.log(
      statusError('Usage: manic plugin <add|remove|list> [package-name]')
    );
    process.exit(1);
  }
  ...
  if (
    ...
    !isValidManicPluginPackageName(pkgName!)
  ) {
    console.log(
      statusError(
  ...
  if (!configPath) {
    console.error(
      statusError('No manic.config.ts/js found in current directory.')
    );
    process.exit(1);
  }
  ...
  console.log(
    statusPending(
      action === 'add' ? `Installing ${pkgName!}...` : `Removing ${pkgName!}...`
    )
  );
  if (action === 'add') {
