...
const commandMap = new Map(
  commandList.map(command => [command.name, command] as const)
);
...
function renderExamplesHelp(): string {
  return commandList
    ...
    .map(example => {
      const prefixed = example.startsWith('manic ')
        ? example.slice('manic '.length)
        : example;
...
function renderCommandHelp(command: CliCommand): string {
  const usage = command.usage
    .map(line => {
      const value = line.startsWith('manic ')
        ? line.slice('manic '.length)
        : line;
