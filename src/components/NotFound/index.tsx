...
function similarity(a: string, b: string): number {
  ...
  return (
    (matches / longer.length) * 0.6 + (sharedSegs / Math.max(segsA.length, segsB.length, 1)) * 0.4
  );
}

/** Dev-friendly fallback 404 shell with route hints. @see https://www.manicjs.tech/docs/api/errors/not-found#props */
export function NotFound({ routes, currentPath }: { routes?: RouteEntry[]; currentPath?: string }) {
      ...
      `}</style>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          >
            {routes ? (
              <>
                You might have mistyped the URL, or the route may not be configured yet. Make sure
          ...
          >
            ...
            >
              ...
              >
                ...
                <tbody>
                  {routes.map((route, i) => {
                    const score = currentPath ? similarity(currentPath, route.path) : 0;
                    const rowColor =
                      score > 0.8
                        ? "rgba(255,184,0,0.08)"
                        : i % 2 !== 0
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          <button
                            onClick={() =>
                              fetch(`/_manic/open?file=${encodeURIComponent(route.file)}`)
                            }
                            style={{
                              background: "none",
                              border: "none",
